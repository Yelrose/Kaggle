...
def siamese_conv(pretrain=False):
    ...
    with tf.device('/cpu:0'):
        if pretrain:
            embedding_input = Embedding(vocab_size,
                                    embedding_dim,
...
feature = Merge(mode='concat', concat_axis=-1)([abs_feature, mul_feature, gru_abs_feature, gru_mul_feature, leaks_input])
...
# train data
X_q1 = np.load("/home/huangzhengjie/quora_pair/quora_stack/stage0_output/train/X_q1_lcs.npy", mmap_mode='r')
X_q2 = np.load("/home/huangzhengjie/quora_pair/quora_stack/stage0_output/train/X_q2_lcs.npy", mmap_mode='r')
...
class hash_deepnet_lcs(object):
    ...
    def train(self, train_index, test_index):
        ...
        final_model.fit([train_q1, train_q1_mask, train_q2, train_q2_mask, train_l],
                        train_y,
                        validation_data=([val_q1, val_q1_mask, val_q2, val_q2_mask, val_l], val_y),
                        batch_size=512,
                        nb_epoch=2)
        val_prediction = final_model.predict([val_q1, val_q1_mask, val_q2, val_q2_mask, val_l], batch_size=512, verbose=1)
        score = log_loss(val_y, np.array(val_prediction,dtype='float64'))
    ...
    def test(self):
        test_q1 = np.load('/home/huangzhengjie/quora_pair/quora_stack/stage0_output/test/X_q1_lcs.npy')[:,:max_sequence_length]
        test_q2 = np.load('/home/huangzhengjie/quora_pair/quora_stack/stage0_output/test/X_q2_lcs.npy')[:,:max_sequence_length]
        test_q1_mask = test_q1 > 0#, test_q1 == embedding_matrix.shape[0] - 1)
        test_q2_mask = test_q2 > 0#, test_q2 == embedding_matrix.shape[0] - 1)
        test_l = test_leaks
        prediction = final_model.predict([test_q1, test_q1_mask, test_q2, test_q2_mask, test_l], batch_size=512, verbose=1)
        return prediction



if __name__ == "__main__":
    model = hash_deepnet_lcs()
    with open("/home/huangzhengjie/quora_pair/quora_stack/kdf.pkl",'r') as f:
        kfd = pkl.load(f)

    X_prediction = np.zeros((X_q1.shape[0], 1), dtype='float64')
    score_log = []
    ...
    np.save("/home/huangzhengjie/quora_pair/quora_stack/stage1_output/train/hash_deepnet_lcs_%s.npy" % state, X_prediction)
    model.train_all()
    pred = model.test()
    np.save("/home/huangzhengjie/quora_pair/quora_stack/stage1_output/test/hash_deepnet_lcs_%s.npy" % state, pred)
